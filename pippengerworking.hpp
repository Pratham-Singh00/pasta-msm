...
namespace pasta_msm {
size_t getwindow(const uint32_t k[4],size_t start_idx){//this assumes a window size of 16
    uint32_t block=(start_idx)/32;
    uint32_t blockidx=start_idx%32;
    uint32_t mask=k[block];
    if(blockidx==16){return (mask>>16);}//last 16 bits of block
    else {return uint16_t(mask);}//first 16 bits of block
}
...
void mult_pippenger_glv(
    ...
    //scalarsp[i][j] -> ith scalar, jth limb, where there are 32 limbs are each one is 8 bits
    constexpr size_t BITS_TO_PROCESS = 128; 
    constexpr size_t W = 16;
    // Number of windows needed for the 128-bit decomposed scalars.
    ...
    for (size_t win = num_windows; win-- > 0;) {
        // Scale the accumulated result by 2^W to make room for the new window's sum.
        // This is the core of Horner's method for polynomial evaluation.
        // Skip for the very first (most significant) window.
        ...
        // Step 4: Reduce the buckets and add to the final result.
        point_t window_sum; window_sum.inf();
        point_t bucket_accumulator; bucket_accumulator.inf();
        // Sum up buckets: sum = B[nb-1] + (B[nb-1]+B[nb-2]) + ...
